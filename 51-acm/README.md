...
## Purpose
Requests and validates a **wildcard TLS certificate** for `*.aiops26.online` so the
public frontend ALB can serve HTTPS.

## What it creates
([main.tf](./main.tf))
- `aws_acm_certificate` for `*.aiops26.online` with **DNS validation**.
...
## Depends on
- A Route53 hosted zone for `aiops26.online` (zone id in [variables.tf](./variables.tf)).
